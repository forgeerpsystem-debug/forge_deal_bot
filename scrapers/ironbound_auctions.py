...
SITE = "Ironbound Auctions"
BASE_URL = "https://www.ironboundauctions.com"


async def scrape_ironbound(keywords: list[str], max_pages: int = 3) -> set[str]:
    ...
    async with async_playwright() as p:
        ...
        for keyword in keywords:
            ...
            for page_num in range(1, max_pages + 1):
                url = f"{BASE_URL}/inventory?q={search_term}&page={page_num}"
                try:
                    await page.goto(url, wait_until="domcontentloaded", timeout=25000)
                    await page.wait_for_selector(
                        "a[href*='/details/'], a[href*='/item/'], a[href*='/lot/'], a[href*='/inventory/']",
                        timeout=8000
                    )
                ...
                links = soup.find_all("a", href=re.compile(r'(?i)/(details|item|lot|inventory)/'))
                if not links:
                    break
                ...
                for link in links:
                    ...
                    try:
                        ...
                        date_elem = container.find(string=re.compile(r'(?i)(closes|ends|auction date)'))
                        if date_elem:
